...
class ReplayBuffer:
    ...
    def get_training_data(self, device):
        ...
        batch = {'s': torch.tensor(self.buffer['s'][:, :self.max_episode_len], dtype=torch.float32, device=device),
                 'a': torch.tensor(self.buffer['a'][:, :self.max_episode_len], dtype=torch.long,device=device),  # 动作a的类型必须是long
                 'a_logprob': torch.tensor(self.buffer['a_logprob'][:, :self.max_episode_len], dtype=torch.float32,device=device),
